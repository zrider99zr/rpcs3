...
const ppu_decoder<ppu_interpreter_precise> s_ppu_interpreter;

PPUTranslator::PPUTranslator(LLVMContext& context, Module* module, u64 base, u64 rtoc)
	: m_context(context)
	, m_module(module)
	, m_base_addr(base)
	, m_is_be(false)
	, m_pure_attr(AttributeSet::get(m_context, AttributeSet::FunctionIndex, std::vector<Attribute::AttrKind>{ Attribute::NoUnwind, Attribute::ReadNone }))
{
	/* Initialize global variables */
	new GlobalVariable(*module, GetType<char*>(), true, GlobalValue::ExternalLinkage, nullptr, "__memory");
}
...
void PPUTranslator::TranslateToIR(u64 start_addr, u64 end_addr, be_t<u32>* bin)
{
	...
	IRBuilder<> builder(BasicBlock::Create(m_context, "entry", m_function));
	m_ir = &builder;

	m_base = m_ir->CreateLoad(m_module->getGlobalVariable("__memory"), false, ".memory");

	// Create context variables
	m_thread = Call(GetType<char*>(), "__context", m_ir->getInt64(start_addr));
	
	m_g_gpr[1] = m_ir->CreateBitCast(m_ir->CreateConstGEP1_32(m_thread, OFFSET_32(PPUThread, GPR[1])), GetType<u64*>(), ".sp");
	m_g_gpr[2] = m_ir->CreateBitCast(m_ir->CreateConstGEP1_32(m_thread, OFFSET_32(PPUThread, GPR[2])), GetType<u64*>(), ".rtoc");
	m_g_gpr[13] = m_ir->CreateBitCast(m_ir->CreateConstGEP1_32(m_thread, OFFSET_32(PPUThread, GPR[13])), GetType<u64*>(), ".tls");

	for (u32 i = 3; i <= 12; i++)
	{
		m_g_gpr[i] = m_ir->CreateBitCast(m_ir->CreateConstGEP1_32(m_thread, OFFSET_32(PPUThread, GPR[i])), GetType<u64*>(), fmt::format(".r%u", i));
	}

	for (u32 i = 1; i <= 13; i++)
	{
		m_g_fpr[i] = m_ir->CreateBitCast(m_ir->CreateConstGEP1_32(m_thread, OFFSET_32(PPUThread, FPR[i])), GetType<f64*>(), fmt::format(".f%u", i));
	}

	for (u32 i = 2; i <= 13; i++)
	{
		m_g_vr[i] = m_ir->CreateBitCast(m_ir->CreateConstGEP1_32(m_thread, OFFSET_32(PPUThread, VR[i])), GetType<u32[4]>()->getPointerTo(), fmt::format(".v%u", i));
	}
	...
	for (u32 i = 0; i < 32; i++) m_vr[i] = m_g_vr[i] ? m_g_vr[i] : m_ir->CreateAlloca(GetType<u32[4]>(), nullptr, fmt::format(".v%d", i));
	...
	m_xer_count = m_ir->CreateAlloca(m_ir->getIntNTy(7), nullptr, "xer.count");
	...
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_ve);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_oe);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_ue);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_ze);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_xe);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_rnh);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_rnh);

	// Load function arguments if necessary
	if (m_function_type)
	{
		u32 g{}, f{}, v{}; // Counts of args of each type

		for (auto& arg : m_function->getArgumentList())
		{
			...
			if (type->isVectorTy())
			{
				...
				if (++v <= 12)
				{
					m_ir->CreateStore(m_ir->CreateBitCast(&arg, GetType<u32[4]>()), m_vr[v + 1]);
				}
				else
				{
					m_ir->CreateStore(&arg, GetStackArg(g, arg.getType()));
				}
			}
			else if (type->isFloatingPointTy())
			{
				...
				if (++f <= 13)
				{
					m_ir->CreateStore(m_ir->CreateFPCast(&arg, GetType<f64>()), m_fpr[f]);
				}
				else
				{
					m_ir->CreateStore(&arg, GetStackArg(g, arg.getType()));
			...
			else
			{
				if (++g <= 8)
				{
					m_ir->CreateStore(m_ir->CreateCast(type->isPointerTy() ? Instruction::PtrToInt : Instruction::ZExt, &arg, GetType<u64>()), m_gpr[g + 2]);
				}
				else
				{
					m_ir->CreateStore(&arg, GetStackArg(g, arg.getType()));
	...
	for (m_current_addr = start_addr; m_current_addr < end_addr;)
	{
		...
		if (m_current_addr == addr)
			m_current_addr += sizeof(u32);
...
{
	// TODO: Allow to specify function call type for an indirect call at specific position
	...
	if (callee_type)
	{
		...
		for (auto& arg : func->getArgumentList())
		{
			...
			{
				g = (g + 2) | 1;
				if (++v > 12) LOG_ERROR(PPU, "0x%llx: TODO: %s(): Vector arg %u:'%s' ignored", m_current_addr, m_function->getName().data(), v, arg.getName().data());
				args.push_back(m_ir->CreateBitCast(m_ir->CreateLoad(m_vr[v + 1]), type));
			}
			else if (type->isFloatingPointTy())
			{
				g = (g + 1);
				if (++f > 13) LOG_ERROR(PPU, "0x%llx: TODO: %s(): FP arg %u:'%s' ignored", m_current_addr, m_function->getName().data(), v, arg.getName().data());
				args.push_back(m_ir->CreateFPCast(m_ir->CreateLoad(m_fpr[f]), type));
			}
			else
			{
				if (++g > 8) LOG_ERROR(PPU, "0x%llx: TODO: %s(): Argument %u:'%s' ignored", m_current_addr, m_function->getName().data(), v, arg.getName().data());
				args.push_back(m_ir->CreateCast(type->isPointerTy() ? Instruction::IntToPtr : Instruction::Trunc, m_ir->CreateLoad(m_gpr[g + 2]), type));
			}
		}
	}
	...
	else
	{
		...
		if (type->isVectorTy())
		{
			m_ir->CreateStore(m_ir->CreateBitCast(result, GetType<u32[4]>()), m_vr[2]);
		}
		else if (type->isFloatingPointTy())
		{
			m_ir->CreateStore(m_ir->CreateFPCast(result, GetType<f64>()), m_fpr[1]);
		}
		else if (type->isPointerTy())
		{
			m_ir->CreateStore(m_ir->CreatePtrToInt(result, GetType<u64>()), m_gpr[3]);
		}
		else if (!type->isVoidTy())
		{
			m_ir->CreateStore(m_ir->CreateZExt(result, GetType<u64>()), m_gpr[3]);
		}
	}
...
void PPUTranslator::UndefineVolatileRegisters()
{
	...
	m_ir->CreateStore(undef_bool, m_xer_ov); // XER.OV
	m_ir->CreateStore(UndefValue::get(m_ir->getIntNTy(7)), m_xer_count);

	//m_ir->CreateStore(undef_bool, m_fpscr_fx); // FPSCR.FX
	//m_ir->CreateStore(undef_bool, m_fpscr_fr); // FPSCR.FR
	//m_ir->CreateStore(undef_bool, m_fpscr_fi); // FPSCR.FI

	//m_ir->CreateStore(undef_bool, m_fpscr_c); // FPSCR.FPRF
	...
	ClearStickyFlags(); // Cannot undef sticky flags because it makes |= op meaningless
}

void PPUTranslator::ClearStickyFlags()
{
	m_ir->CreateStore(m_ir->getFalse(), m_xer_so); // XER.SO
	m_ir->CreateStore(m_ir->getFalse(), m_vscr_sat); // VSCR.SAT

	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_ox);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_ux);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_zx);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_xx);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxsnan);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxisi);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxidi);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxzdz);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vximz);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxvc);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxsoft);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxsqrt);
	//m_ir->CreateStore(m_ir->getFalse(), m_fpscr_vxcvi);
}

void PPUTranslator::PrepareGlobalArguments()
{
}

void PPUTranslator::UpdateLocalResults()
{
}

void PPUTranslator::ReturnFromFunction()
{
	if (!m_function_type)
	{
		m_ir->CreateRetVoid();
	...
	const auto type = m_function_type->getReturnType();
	...
	{
		m_ir->CreateRet(m_ir->CreateBitCast(m_ir->CreateLoad(m_vr[2]), type));
	}
	else if (type->isFloatingPointTy())
	{
		m_ir->CreateRet(m_ir->CreateFPCast(m_ir->CreateLoad(m_fpr[1]), type));
	}
	else if (type->isPointerTy())
	{
		m_ir->CreateRet(m_ir->CreateIntToPtr(m_ir->CreateLoad(m_gpr[3]), type));
	}
	else
	{
		m_ir->CreateRet(m_ir->CreateTrunc(m_ir->CreateLoad(m_gpr[3]), type));
	}
}
...
{
	return m_ir->CreateBitCast(value, m_ir->getIntNTy(value->getType()->getPrimitiveSizeInBits()));
}
...
void PPUTranslator::UseCondition(Value* cond)
{
	if (cond)
	{
		const auto local = BasicBlock::Create(m_ir->getContext(), fmt::format("loc_%llx.cond", m_current_addr/* - m_start_addr*/), m_function);
		m_ir->CreateCondBr(cond, local, GetBasicBlock(m_current_addr + 4));
		m_ir->SetInsertPoint(local);
	}
}

Value* PPUTranslator::ReadMemory(Value* addr, Type* type, bool is_be)
{
	...
	if (is_be ^ m_is_be && size > 8)
	{
		...
		const auto value = m_ir->CreateLoad(m_ir->CreateBitCast(m_ir->CreateGEP(m_base, addr), int_type->getPointerTo()), !IsStackAddr(addr));
		return m_ir->CreateBitCast(Call(int_type, fmt::format("llvm.bswap.i%u", size), value), type);
	}

	// Read normally
	return m_ir->CreateLoad(m_ir->CreateBitCast(m_ir->CreateGEP(m_base, addr), type->getPointerTo()), !IsStackAddr(addr));
}

void PPUTranslator::WriteMemory(Value* addr, Value* value, bool is_be)
{
	const auto type = value->getType();
	const auto size = type->getPrimitiveSizeInBits();
	...
	// Write
	m_ir->CreateStore(value, m_ir->CreateBitCast(m_ir->CreateGEP(m_base, addr), value->getType()->getPointerTo()), !IsStackAddr(addr));
}
...
void PPUTranslator::InterpreterCall(ppu_opcode_t op)
{
	const auto ptr = m_ir->getInt64((u64)s_ppu_interpreter.decode(op.opcode));
	const auto fty = FunctionType::get(GetType<void>(), std::vector<llvm::Type*>{ m_thread->getType(), GetType<u32>() }, false)->getPointerTo();
	m_ir->CreateCall2(m_ir->CreateIntToPtr(ptr, fty), m_thread, m_ir->getInt32(op.opcode));
}
...
{
	const auto ab = GetVrs(VrType::vf, op.va, op.vb);
	const auto le = m_ir->CreateFCmpOLE(ab[0], ab[1]);
	const auto ge = m_ir->CreateFCmpOGE(ab[0], m_ir->CreateFNeg(ab[1]));
	const auto le_bit = m_ir->CreateShl(ZExt(le, GetType<u32[4]>()), 31);
	const auto ge_bit = m_ir->CreateShl(ZExt(ge, GetType<u32[4]>()), 30);
	const auto result = m_ir->CreateXor(m_ir->CreateOr(le_bit, ge_bit), 0xc0000000); // TODO
	SetVr(op.vd, result);
...
void PPUTranslator::VMADDFP(ppu_opcode_t op)
{
	...
	SetVr(op.vd, Call(GetType<f32[4]>(), "llvm.fma.v4f32", acb[0], acb[1], acb[2]));
}

void PPUTranslator::VMAXFP(ppu_opcode_t op)
{
	const auto ab = GetVrs(VrType::vf, op.va, op.vb);
	SetVr(op.vd, Call(GetType<f32[4]>(), "llvm.maxnum.v4f32", ab[0], ab[1]));
}
...
{
	const auto ab = GetVrs(VrType::vf, op.va, op.vb);
	SetVr(op.vd, Call(GetType<f32[4]>(), "llvm.minnum.v4f32", ab[0], ab[1]));
}
...
void PPUTranslator::VMSUMMBM(ppu_opcode_t op)
{
	...
	const auto e0 = Shuffle(p, {}, { 0, 4, 8, 12 });
	const auto e1 = Shuffle(p, {}, { 1, 5, 9, 13 });
	const auto e2 = Shuffle(p, {}, { 2, 6, 10, 14 });
	const auto e3 = Shuffle(p, {}, { 3, 7, 11, 15 });
...
void PPUTranslator::VMSUMSHM(ppu_opcode_t op)
{
	...
	const auto c = GetVr(op.vc, VrType::vi32);
	const auto e0 = Shuffle(p, {}, { 0, 2, 4, 6 });
	const auto e1 = Shuffle(p, {}, { 1, 3, 5, 7 });
...
{
	const auto acb = GetVrs(VrType::vf, op.va, op.vc, op.vb);
	SetVr(op.vd, m_ir->CreateFNeg(Call(GetType<f32[4]>(), "llvm.fma.v4f32", acb[0], acb[1], m_ir->CreateFNeg(acb[2]))));
}
...
void PPUTranslator::VSPLTB(ppu_opcode_t op)
{
	...
	SetVr(op.vd, Shuffle(GetVr(op.vb, VrType::vi8), {}, { ui, ui, ui, ui, ui, ui, ui, ui, ui, ui, ui, ui, ui, ui, ui, ui }));
}

void PPUTranslator::VSPLTH(ppu_opcode_t op)
{
	const u32 ui = op.vuimm & 0x7;
	SetVr(op.vd, Shuffle(GetVr(op.vb, VrType::vi16), {}, { ui, ui, ui, ui, ui, ui, ui, ui }));
}
...
void PPUTranslator::VSPLTW(ppu_opcode_t op)
{
	...
	SetVr(op.vd, Shuffle(GetVr(op.vb, VrType::vi32), {}, { ui, ui, ui, ui }));
}
...
void PPUTranslator::VSUM2SWS(ppu_opcode_t op)
{
	// TODO (rare)
	const auto ab = GetVrs(VrType::vi32, op.va, op.vb);
	const auto b = SExt(Shuffle(ab[1], {}, { 1, 3 }));
	const auto a = SExt(ab[0]);
	const auto e0 = Shuffle(a, {}, { 0, 2 });
	const auto e1 = Shuffle(a, {}, { 1, 3 });
...
void PPUTranslator::VSUM4UBS(ppu_opcode_t op)
{
	...
	const auto b = GetVr(op.vb, VrType::vi32);
	const auto e0 = Shuffle(a, {}, { 0, 4, 8, 12 });
	const auto e1 = Shuffle(a, {}, { 1, 5, 9, 13 });
	const auto e2 = Shuffle(a, {}, { 2, 6, 10, 14 });
	const auto e3 = Shuffle(a, {}, { 3, 7, 11, 15 });
...
{
	const auto px = SExt(Shuffle(GetVr(op.vb, VrType::vi16), {}, { 0, 1, 2, 3 }));
	SetVr(op.vd, UNPACK_PIXEL_OP(px));
}

void PPUTranslator::VUPKHSB(ppu_opcode_t op)
{
	SetVr(op.vd, SExt(Shuffle(GetVr(op.vb, VrType::vi8), {}, { 0, 1, 2, 3, 4, 5, 6, 7 })));
}

void PPUTranslator::VUPKHSH(ppu_opcode_t op)
{
	SetVr(op.vd, SExt(Shuffle(GetVr(op.vb, VrType::vi16), {}, { 0, 1, 2, 3 })));
}

void PPUTranslator::VUPKLPX(ppu_opcode_t op)
{
	const auto px = SExt(Shuffle(GetVr(op.vb, VrType::vi16), {}, { 4, 5, 6, 7 }));
	SetVr(op.vd, UNPACK_PIXEL_OP(px));
}

void PPUTranslator::VUPKLSB(ppu_opcode_t op)
{
	SetVr(op.vd, SExt(Shuffle(GetVr(op.vb, VrType::vi8), {}, { 8, 9, 10, 11, 12, 13, 14, 15 })));
}

void PPUTranslator::VUPKLSH(ppu_opcode_t op)
{
	SetVr(op.vd, SExt(Shuffle(GetVr(op.vb, VrType::vi16), {}, { 4, 5, 6, 7 })));
}
...
void PPUTranslator::SUBFIC(ppu_opcode_t op)
{
	...
	const auto result = m_ir->CreateSub(imm, GetGpr(op.ra));
	SetGpr(op.rd, result);
...
{
	const auto imm = m_ir->getInt64(op.simm16);
	const auto result = m_ir->CreateAdd(GetGpr(op.ra), imm);
	SetGpr(op.rd, result);
...
{
	PrepareGlobalArguments();
	Call(GetType<void>(), "__hack", m_ir->getInt32(op.opcode & 0x3ffffff));
	UndefineVolatileRegisters();
	UpdateLocalResults();
}
...
{
	PrepareGlobalArguments();
	Call(GetType<void>(), fmt::format(op.lev == 0 ? "__syscall" : "__lv%u_syscall", +op.lev), m_ir->CreateLoad(m_gpr[11]));
	UndefineVolatileRegisters();
	UpdateLocalResults();
}
...
void PPUTranslator::BCCTR(ppu_opcode_t op)
{
	...
	const auto ctr = m_ir->CreateLoad(m_reg_ctr);
		...
		if (target % 4 || target < m_start_addr || target >= m_end_addr)
		{
			if (i >= 2)
			{
				...
				if (!op.lk)
				{
					...
					const auto _default = BasicBlock::Create(m_ir->getContext(), fmt::format("loc_%llx.def", m_current_addr/* - m_start_addr*/), m_function);
					const auto _switch = m_ir->CreateSwitch(ctr, _default, ::size32(cases));
	...
	// Plain indirect jump (probably not a jumptable)
	CallFunction(0, !op.lk, ctr);
}
...
void PPUTranslator::LVEBX(ppu_opcode_t op)
{
	...
	SetVr(op.vd, m_ir->CreateInsertElement(GetUndef<u8[16]>(), ReadMemory(addr, GetType<u8>()), m_ir->CreateXor(m_ir->CreateAnd(addr, 15), m_is_be ? 0 : 15)));
}
...
void PPUTranslator::MFOCRF(ppu_opcode_t op)
{
	...
	SetGpr(op.rd, result ? result : GetUndef<u64>());
}
...
void PPUTranslator::SLD(ppu_opcode_t op)
{
	...
	const auto shift_res = m_ir->CreateShl(GetGpr(op.rs), shift_num);
	const auto result = m_ir->CreateSelect(m_ir->CreateICmpUGT(shift_num, m_ir->getInt64(0x3f)), m_ir->getInt64(0), shift_res);
	SetGpr(op.ra, result);
	if (op.rc) SetCrFieldSignedCmp(0, result, m_ir->getInt64(0));
}
...
void PPUTranslator::LVEHX(ppu_opcode_t op)
{
	...
	SetVr(op.vd, m_ir->CreateInsertElement(GetUndef<u16[8]>(), ReadMemory(addr, GetType<u16>()), m_ir->CreateLShr(m_ir->CreateXor(m_ir->CreateAnd(addr, 15), m_is_be ? 0 : 15), 1)));
}
...
void PPUTranslator::LVEWX(ppu_opcode_t op)
{
	...
	SetVr(op.vd, m_ir->CreateInsertElement(GetUndef<u32[4]>(), ReadMemory(addr, GetType<u32>()), m_ir->CreateLShr(m_ir->CreateXor(m_ir->CreateAnd(addr, 15), m_is_be ? 0 : 15), 2)));
}
...
{
	SetVr(op.vd, ReadMemory(m_ir->CreateAnd(op.ra ? m_ir->CreateAdd(GetGpr(op.ra), GetGpr(op.rb)) : GetGpr(op.rb), -16), GetType<u32[4]>()));
}
...
void PPUTranslator::SUBFE(ppu_opcode_t op)
{
	...
	SetGpr(op.rd, result);
	SetCarry(Call(GetType<bool>(), m_pure_attr, "__adde_get_carry", a, b, c));
	if (op.rc) SetCrFieldSignedCmp(0, result, m_ir->getInt64(0));
...
{
	const auto addr = m_ir->CreateAnd(op.ra ? m_ir->CreateAdd(GetGpr(op.ra), GetGpr(op.rb)) : GetGpr(op.rb), -2);
	WriteMemory(addr, m_ir->CreateExtractElement(GetVr(op.vs, VrType::vi16), m_ir->CreateLShr(m_ir->CreateXor(m_ir->CreateAnd(addr, 15), m_is_be ? 0 : 15), 1)));
}
...
{
	const auto addr = m_ir->CreateAnd(op.ra ? m_ir->CreateAdd(GetGpr(op.ra), GetGpr(op.rb)) : GetGpr(op.rb), -4);
	WriteMemory(addr, m_ir->CreateExtractElement(GetVr(op.vs, VrType::vi32), m_ir->CreateLShr(m_ir->CreateXor(m_ir->CreateAnd(addr, 15), m_is_be ? 0 : 15), 2)));
}
...
{
	WriteMemory(m_ir->CreateAnd(op.ra ? m_ir->CreateAdd(GetGpr(op.ra), GetGpr(op.rb)) : GetGpr(op.rb), -16), GetVr(op.vs, VrType::vi32));
}
...
{
	SetGpr(op.rd, Call(GetType<u64>(), "__eciwx", m_ir->CreateIntToPtr(op.ra ? m_ir->CreateAdd(GetGpr(op.ra), GetGpr(op.rb)) : GetGpr(op.rb), GetType<u32*>())));
}
...
{
	const auto result = m_ir->CreateXor(GetGpr(op.rs), GetGpr(op.rb));
	SetGpr(op.ra, result);
	SetCrFieldSignedCmp(0, result, m_ir->getInt64(0));
}
...
{
	const auto result = m_ir->CreateOr(GetGpr(op.rs), m_ir->CreateNot(GetGpr(op.rb)));
	SetGpr(op.ra, result);
	if (op.rc) SetCrFieldSignedCmp(0, result, m_ir->getInt64(0));
}
...
{
	Call(GetType<void>(), "__ecowx", m_ir->CreateIntToPtr(op.ra ? m_ir->CreateAdd(GetGpr(op.ra), GetGpr(op.rb)) : GetGpr(op.rb), GetType<u32*>()), GetGpr(op.rs, 32));
}
...
void PPUTranslator::DIVDU(ppu_opcode_t op)
{
	...
	const auto result = m_ir->CreateSelect(o, m_ir->getInt64(0), m_ir->CreateUDiv(a, m_ir->CreateSelect(o, m_ir->getInt64(1), b)));
	SetGpr(op.rd, result);
	if (op.rc) SetCrFieldSignedCmp(0, result, m_ir->getInt64(0));
...
void PPUTranslator::DIVWU(ppu_opcode_t op)
{
	...
	const auto o = IsZero(b);
	SetGpr(op.rd, m_ir->CreateSelect(o, m_ir->getInt32(0), m_ir->CreateUDiv(a, m_ir->CreateSelect(o, m_ir->getInt32(1), b))));
	if (op.rc) SetCrField(0, GetUndef<bool>(), GetUndef<bool>(), GetUndef<bool>());
	if (op.oe) SetOverflow(o);
}
...
void PPUTranslator::MTSPR(ppu_opcode_t op)
{
	...
	case 0x001: // MTXER
		...
		m_ir->CreateStore(Trunc(value, m_ir->getIntNTy(7)), m_xer_count);
		break;
	case 0x008: // MTLR
...
void PPUTranslator::DIVD(ppu_opcode_t op)
{
	...
	const auto result = m_ir->CreateSelect(o, m_ir->getInt64(0), m_ir->CreateSDiv(a, m_ir->CreateSelect(o, m_ir->getInt64(1), b)));
	SetGpr(op.rd, result);
	if (op.rc) SetCrFieldSignedCmp(0, result, m_ir->getInt64(0));
	if (op.oe) SetOverflow(o);
}
...
void PPUTranslator::DIVW(ppu_opcode_t op)
{
	...
	SetGpr(op.rd, m_ir->CreateSelect(o, m_ir->getInt32(0), m_ir->CreateSDiv(a, m_ir->CreateSelect(o, m_ir->getInt32(1), b))));
	if (op.rc) SetCrField(0, GetUndef<bool>(), GetUndef<bool>(), GetUndef<bool>());
	if (op.oe) SetOverflow(o);
}
...
{
	const auto shift_num = m_ir->CreateAnd(GetGpr(op.rb), 0x7f);
	const auto shift_res = m_ir->CreateLShr(GetGpr(op.rs), shift_num);
	const auto result = m_ir->CreateSelect(m_ir->CreateICmpUGT(shift_num, m_ir->getInt64(0x3f)), m_ir->getInt64(0), shift_res);
	SetGpr(op.ra, result);
	if (op.rc) SetCrFieldSignedCmp(0, result, m_ir->getInt64(0));
}
...
{
	const auto shift_num = m_ir->CreateAnd(GetGpr(op.rb), 0x3f);
	const auto shift_arg = SExt(GetGpr(op.rs, 32));
	const auto result = m_ir->CreateAShr(shift_arg, shift_num);
	SetGpr(op.ra, result);
...
void PPUTranslator::SRAD(ppu_opcode_t op)
{
	...
	const auto shift_max = m_ir->getInt64(0x3f);
	const auto overshift = m_ir->CreateICmpUGT(shift_num, shift_max);
	const auto result = m_ir->CreateAShr(shift_arg, m_ir->CreateSelect(overshift, shift_max, shift_num));
	SetGpr(op.ra, result);
	const auto carry_mask = m_ir->CreateNot(m_ir->CreateShl(m_ir->getInt64(-1), shift_num));
...
{
	Call(GetType<void>(), "llvm.memset.p0i8.i32",
		m_ir->CreateIntToPtr(m_ir->CreateAnd(op.ra ? m_ir->CreateAdd(GetGpr(op.ra), GetGpr(op.rb)) : GetGpr(op.rb), -128), GetType<u8*>()),
		m_ir->getInt8(0),
		m_ir->getInt32(128),
		m_ir->getInt32(128),
		m_ir->getTrue());
...
{
	const auto addr = m_ir->CreateAdd(GetGpr(op.ra), m_ir->getInt64(op.ds << 2));
	WriteMemory(addr, GetGpr(op.rs, 64));
	SetGpr(op.ra, addr);
}
...
void PPUTranslator::FMADDS(ppu_opcode_t op)
{
	...
	const auto b = GetFpr(op.frb);
	const auto c = GetFpr(op.frc);
	const auto result = m_ir->CreateFPTrunc(Call(GetType<f64>(), "llvm.fma.f64", a, c, b), GetType<f32>());
	SetFpr(op.frd, result);
...
{
	const auto a = GetFpr(op.fra);
	const auto b = m_ir->CreateFNeg(GetFpr(op.frb));
	const auto c = GetFpr(op.frc);
	const auto result = m_ir->CreateFPTrunc(Call(GetType<f64>(), "llvm.fma.f64", a, c, b), GetType<f32>());
	SetFpr(op.frd, result);
...
void PPUTranslator::FNMSUBS(ppu_opcode_t op)
{
	...
	const auto b = m_ir->CreateFNeg(GetFpr(op.frb));
	const auto c = GetFpr(op.frc);
	const auto result = m_ir->CreateFPTrunc(m_ir->CreateFNeg(Call(GetType<f64>(), "llvm.fma.f64", a, c, b)), GetType<f32>());
	SetFpr(op.frd, result);
...
void PPUTranslator::FMSUB(ppu_opcode_t op)
{
	...
	const auto b = m_ir->CreateFNeg(GetFpr(op.frb));
	const auto c = GetFpr(op.frc);
	const auto result = Call(GetType<f64>(), "llvm.fma.f64", a, c, b);
	SetFpr(op.frd, result);
...
void PPUTranslator::FNMSUB(ppu_opcode_t op)
{
	...
	const auto b = m_ir->CreateFNeg(GetFpr(op.frb));
	const auto c = GetFpr(op.frc);
	const auto result = m_ir->CreateFNeg(Call(GetType<f64>(), "llvm.fma.f64", a, c, b));
	SetFpr(op.frd, result);
...
void PPUTranslator::SetGpr(u32 r, Value* val_x64)
{
	// Get i64 value (zero-extend if necessary)
	const auto value = m_ir->CreateZExt(val_x64, GetType<u64>());

	if (r != 2 && r != 13)
	{
		// Update local variable except RTOC, TLS
		m_ir->CreateStore(value, m_gpr[r]);
	}

	if (r == 1 || r == 2 || r == 13)
	{
		// Update global variable for SP, RTOC, TLS
		m_ir->CreateStore(value, m_g_gpr[r]);
	}
...
{
	const auto value = m_ir->CreateLoad(m_fpr[r]);
	...
	{
		const auto int_value = m_ir->CreateBitCast(value, GetType<u64>());

		if (bits == 32)
		{
			return m_ir->CreateTrunc(int_value, GetType<u32>());
		}
		else
		{
			VERIFY(bits == 64);
...
void PPUTranslator::SetVr(u32 vr, Value* value)
{
	...
	m_ir->CreateStore(m_ir->CreateBitCast(value, GetType<u32[4]>()), m_vr[vr]);
	m_value_usage[m_vr[vr]]++;
}

Value* PPUTranslator::GetStackArg(u32 g_count, Type* type)
{
	// TODO
	const auto sp = m_ir->CreateLoad(m_gpr[1]); // Load SP
	const auto off = m_ir->CreateAdd(sp, m_ir->getInt64((0x30 + (g_count - 1) * 8) & (type->isVectorTy() ? -16 : -8) + (type->isFloatTy() ? 4 : 0)));
	return m_ir->CreateIntToPtr(off, type->isVectorTy() || type->isFloatingPointTy() ? type->getPointerTo() : GetType<u64*>());
}
...
void PPUTranslator::SetFPSCR_FR(llvm::Value* value)
{
	m_ir->CreateStore(value, m_fpscr_fr);
}

void PPUTranslator::SetFPSCR_FI(llvm::Value* value)
{
	m_ir->CreateStore(value, m_fpscr_fi);
	SetFPSCRException(m_fpscr_xx, value);
}

void PPUTranslator::SetFPSCRException(llvm::Value* ptr, llvm::Value* value)
{
...
llvm::Value* PPUTranslator::GetFPSCRBit(u32 n)
{
	if (n == 1 && m_fpscr[24])
	{
...
void PPUTranslator::SetFPSCRBit(u32 n, llvm::Value* value, bool update_fx)
{
	if (n >= 32 || !m_fpscr[n])
	{
...
llvm::Value* PPUTranslator::Trap(u64 addr)
{
...
bool PPUTranslator::IsStackAddr(llvm::Value* addr)
{
