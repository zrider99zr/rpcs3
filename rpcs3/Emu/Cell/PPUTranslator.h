...
class PPUTranslator final //: public CPUTranslator
{
	...
	// LLVM function attribute set for "custom" intrinsics which are "pure" and may be optimized away if their results are unused
	...
	// Set sticky flags to zero
	void ClearStickyFlags();

	// Update global variables to pass function arguments
	void PrepareGlobalArguments();

	// Load function results from global variables
	void UpdateLocalResults();

	// Set function result, emit ret
	...
	// Get pointer to the parameter save area
	llvm::Value* GetStackArg(u32 g_count, llvm::Type*);
	...
	// Read from memory
	llvm::Value* ReadMemory(llvm::Value* addr, llvm::Type* type, bool is_be = true);

	// Write to memory
	void WriteMemory(llvm::Value* addr, llvm::Value* value, bool is_be = true);
	...
	{
		// Call the function
		return m_ir->CreateCall(m_module->getOrInsertFunction(name, llvm::FunctionType::get(ret, std::vector<llvm::Type*>{ args->getType()... }, false), attr), std::vector<llvm::Value*>{ args... });
	}
...
public:
	PPUTranslator(llvm::LLVMContext& context, llvm::Module* module, u64 base, u64 rtoc);
	~PPUTranslator();
	...
	void TranslateToIR(u64 start_addr, u64 end_addr, be_t<u32>* bin);
